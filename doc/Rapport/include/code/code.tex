\chapter{Aspects techniques}
	...
	\section{Représentation d'un livre}
		...
		\subsection{Représentation des noeuds}
			Il y a quatre type de noeud représenté par des classe: \textbf{BookNodeWithChoices}, \textbf{BookNodeWithRandomChoices}, \textbf{BookNodeCombat} et \textbf{BookNodeTerminal}. Tous extends direct ou indirectement de \textbf{AbstractBookNode}. Cette dernière contient juste un texte pour le constructeur car toutes les méthodes contiennent au moins un texte.\\
...
\end{lstlisting}
	...
	\section{Edition graphique d'un livre}
		...
		\begin{figure}[H]
			\centering\includegraphics[width=0.50\textwidth]{img/MainWindows.png}
			\caption{MainWindows}
		...
		La deuxième partie du TabPane contient un Accordion.  Cette accordion est affiché grâce au bouton \textbf{Ajouter}. Chaque accordion est composé d'un CharacterCreationComponent comprenant un Texte, une CheckBox (Texte, Item), et d'un bouton \textbf{Supprimer cette partie}. La checkBox n'affiche que \textbf{Texte} si aucun item n'est disponible. Mais si un item est créé, l'utilisateur peu alors ajouter un ItemListComponent\ref{ItemListComponent} en sélectionnant \textbf{Item} dans la CheckBox. Cette classe est affiché sur tout les noeuds, permettant d'ajouter des items ainsi que la quantité disponible de chaque item, qui ici, sera disponible en début de partie. L'item ajouté peut être supprimé grâce à un clic droit de la souris.\\
